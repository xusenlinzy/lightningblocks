...
from lightningblocks.utils.seed import seed_everything
from lightningblocks.callbacks import Logger, tqdm
from lightningblocks.task.uie.model import UIE
from lightningblocks.task.uie.evaluate import evaluate
from tqdm.contrib.logging import logging_redirect_tqdm
from lightningblocks.task.uie.utils import IEDataset, SpanEvaluator, EarlyStopping
...
def do_train(args):
    ...
    if args.early_stopping:
        ...
        early_stopping = EarlyStopping(
            patience=4, verbose=True, trace_func=trace_func,
    ...
    if show_bar:
        train_postfix_info = {'losses': 'unknown'}
        ...
        for batch in train_data_iterator:
            ...
            if global_step % args.logging_steps == 0:
                ...
                if show_bar:
                    with logging_redirect_tqdm([logger.logger]):
                        logger.info(
                            "global step %d, epoch: %d, losses: %.5f, speed: %.2f step/s"
            ...
            if global_step % args.valid_steps == 0:
                ...
                if show_bar:
                    train_postfix_info.update({
                        'F1': f'{f1:.3f}',
                        'dev losses': f'{dev_loss_avg:.5f}'
                    })
                    train_data_iterator.set_postfix(train_postfix_info)
                    with logging_redirect_tqdm([logger.logger]):
                        logger.info("Evaluation precision: %.5f, recall: %.5f, F1: %.5f, dev losses: %.5f"
