...
class Vocab(object):
    ...
    def __init__(self,
        ...
        for special_token_name in special_iter:
            ...
            if not special_token_name.endswith('_token'):
                raise ValueError(
                    f"{special_token_name} is invalid. Only keyword arguments that end in \'_token\' are supported to declare special tokens.")
        ...
        if counter is None:
            ...
            for special_token in special_tokens:
                assert special_token in token_to_idx, f'{special_token} is not in token_to_idx'
        ...
        for identifier, token in kwargs.items():
            ...
            if hasattr(self, identifier):
                raise ValueError(
                    f'vocab.{identifier} already exists. Please choose a different identifier for token {token}')
    ...
    def _sort_index_according_to_user_specification(self, token_to_idx):
        ...
        if min(token_to_idx.values()) < 0 or max(token_to_idx.values()) >= len(
                self.token_to_idx):
            raise ValueError(
                f'User-specified indices must not be < 0 or >= the number of tokens that will be in the vocabulary. The current vocab contains {len(self.token_to_idx)}tokens.')
    ...
    def to_tokens(self, indices):
        ...
        Example:
            .. code-block:: python
                from lightningblocks.data import Vocab
        ...
        if isinstance(indices, np.ndarray) and len(indices.shape) > 1:
            raise ValueError(f'Token indices is invalid. Expected 1D array, but received {len(indices.shape)}D array. ')
        ...
        for idx in indices:
            ...
            try:
                ...
                raise ValueError(f'Token index {idx} in the provided `indices` is invalid.') from e
    ...
    def to_json(self, path=None):
        ...
        """
        vocab_dict = {'idx_to_token': dict(self.idx_to_token), 'token_to_idx': dict(self.token_to_idx),
                      'unk_token': self.unk_token, 'identifiers_to_tokens': self._identifiers_to_tokens}
    ...
    def from_json(cls, json_str):
        ...
        return cls(counter=None, token_to_idx=token_to_idx, unk_token=unk_token, **identifiers_to_tokens)
    ...
    def from_dict(
        ...
        Example:
            .. code-block:: python
                ...
                # 1256608 1256608 1256608
        """
        return cls(counter=None, token_to_idx=token_to_idx, unk_token=unk_token, pad_token=pad_token,
    ...
    def build_vocab(iterator,
        ...
        return Vocab(counter, max_size=max_size, min_freq=min_freq, token_to_idx=token_to_idx, unk_token=unk_token,
    ...
    def load_vocabulary(filepath,
        ...
        return Vocab.from_dict(token_to_idx, unk_token=unk_token, pad_token=pad_token, bos_token=bos_token,
                               eos_token=eos_token, **kwargs)
