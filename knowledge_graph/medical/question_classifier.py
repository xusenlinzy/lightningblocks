...
LOGGER = Logger("Question Classifier")
...
class QuestionClassifier:
    def __init__(self):
        # 加载特征词
        self.disease_wds = read("dict/disease.txt")
        self.department_wds = read("dict/department.txt")
        self.check_wds = read("dict/check.txt")
        self.drug_wds = read("dict/drug.txt")
        self.food_wds = read("dict/food.txt")
        self.producer_wds = read("dict/producer.txt")
        self.symptom_wds = read("dict/symptom.txt")
        ...
        self.deny_words = read("dict/deny.txt")
        ...
        self.cause_qwds = ['原因', '成因', '为什么', '怎么会', '怎样才', '咋样才', '怎样会', '如何会',
                           '为啥', '为何', '如何才会', '怎么才会', '会导致', '会造成']

        self.acompany_qwds = ['并发症', '并发', '一起发生', '一并发生', '一起出现', '一并出现',
                              '一同发生', '一同出现', '伴随发生', '伴随', '共现']

        self.food_qwds = ['饮食', '饮用', '吃', '食', '伙食', '膳食', '喝', '菜', '忌口', '补品', '保健品',
                          '食谱', '菜谱', '食用', '食物', '补品']

        self.drug_qwds = ['药', '药品', '用药', '胶囊', '口服液', '炎片']

        self.prevent_qwds = ['预防', '防范', '抵制', '抵御', '防止', '躲避', '逃避', '避开', '免得', '逃开', '避开',
                             '避掉', '躲开', '躲掉', '绕开', '怎样才能不', '怎么才能不', '咋样才能不', '咋才能不', '如何才能不',
        ...
        self.cureway_qwds = ['怎么治疗', '如何医治', '怎么医治', '怎么治', '怎么医', '如何治',
                             '医治方式', '疗法', '咋治', '怎么办', '咋办', '咋治']
        ...
        LOGGER.info(">>> Model Initializing Finished")
    ...
    def check_medical(self, question):
        ...
        for wd1 in region_wds:
            stop_wds.extend(wd1 for wd2 in region_wds if wd1 in wd2 and wd1 != wd2)
        final_wds = [i for i in region_wds if i not in stop_wds]
        return {i: self.wdtype_dict.get(i) for i in final_wds}
