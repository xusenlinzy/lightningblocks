...
def reader(data_path, max_seq_len=512):
    # sourcery skip: identity-comprehension, merge-duplicate-blocks, remove-redundant-if
    """
    read json
    """
...
def convert_cls_examples(raw_examples, prompt_prefix, options):
    ...
    with tqdm(total=len(raw_examples)) as pbar:
        ...
        for line in raw_examples:
            ...
            text, labels = (items["data"], items["label"]) if "data" in items.keys() else (
            ...
            for option in options:
                prompt += option
                prompt += sep
            prompt = f"{prompt_prefix}[{prompt.rstrip(sep)}]"
            result_list = []
...
def add_negative_example(examples, texts, prompts, label_set, negative_ratio):
    ...
    with tqdm(total=len(prompts)) as pbar:
        for i, prompt in enumerate(prompts):
            negative_examples = []
            redundants_list = sorted(set(label_set) ^ set(prompt))
            ...
            if actual_ratio <= negative_ratio or negative_ratio == -1:
                idxs = list(range(len(redundants_list)))
            else:
                idxs = random.sample(range(len(redundants_list)), negative_ratio * num_positive)
...
def add_full_negative_example(examples, texts, relation_prompts, predicate_set, subject_goldens):
    with tqdm(total=len(relation_prompts)) as pbar:
        for i, relation_prompt in enumerate(relation_prompts):
            negative_sample = []
            for subject, predicate in itertools.product(subject_goldens[i], predicate_set):
...
def construct_relation_prompt_set(entity_name_set, predicate_set):
    ...
    for entity_name, predicate in itertools.product(entity_name_set, predicate_set):
        # The relation prompt is constructed as follows:
        # subject + "的" + predicate
        relation_prompt = f"{entity_name}的{predicate}"
...
def convert_ext_examples(raw_examples, negative_ratio, is_train=True):
    ...
    def concat_examples(positive_examples, negative_examples, negative_ratio):
        ...
        if math.ceil(len(negative_examples) /
            ...
            idxs = random.sample(range(len(negative_examples)), negative_ratio * len(positive_examples))
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        """Saves model when validation losses decrease."""
        if self.verbose:
            self.trace_func(
                f'Validation losses decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
