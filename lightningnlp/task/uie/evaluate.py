...
def do_eval(args):
    ...
    test_ds = IEDataset(args.test_path, tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    ...
    for key in class_dict.keys():
        if args.debug:
            test_ds = IEMapDataset(class_dict[key], tokenizer=tokenizer, max_seq_len=args.max_seq_len)
        else:
    ...
    if args.debug and relation_type_dict is not None:
        for key in relation_type_dict.keys():
            test_ds = IEMapDataset(relation_type_dict[key], tokenizer=tokenizer, max_seq_len=args.max_seq_le)
            test_data_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False)
            metric = SpanEvaluator()
            precision, recall, f1 = evaluate(model, metric, test_data_loader, args.device)
